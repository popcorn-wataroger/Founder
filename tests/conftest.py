...
#
# users を入れていない理由（重要）:
#     users は社員マスタで、init_db() が data/users.csv から一度だけ投入する
#     「テストの前提データ」。ここに入れて毎回空にすると、
#     init_db() は「空なら投入する」判定なので毎テスト入れ直すことになり、
#     そのぶん遅くなるうえ、EMP001 でログインするテストが
#     TRUNCATE と投入の間の状態に左右されやすくなる。
#     テストが作ったデータ（ソース・チャット・ログイン記録・上書き）だけを消し、
#     社員マスタは置いたままにする。
TEST_TABLES = (
    "sources",
    "chat_sessions",
...
def temp_db() -> Iterator[None]:
    ...
    処理:
        ...
        2. init_db() でテーブルを用意する（既にあれば何も起きない）。
           このとき users が空なら data/users.csv の社員マスタも投入される
        3. TEST_TABLES の6テーブルを空にして、IDの採番も1に戻す（users は消さない）
        4. テストへ処理を渡す
