...
async def chat(req: ChatRequest, token: dict = Depends(verify_token)):
    ...
    入力:
        ...
        token … verify_token が返すログイン情報（user_id を含む。社員・社長どちらでも可）
    ...
    処理:
        ...
        2. トークンから user_id を取り出し、チャットセッションを1件作る
        3. rag.answer_question に質問を渡して回答を生成する
    ...
    # 2. ログイン情報（JWTの中身）から、誰の会話かを示す user_id を取り出す
    ...
    # 3. RAGの中核関数に質問を渡し、社内文書に基づいた回答を生成してもらう
    ...
    # 5. 生成した回答をJSONで返す（キー名はフロント chat.js の data.reply に合わせる）
    return {"reply": answer}
